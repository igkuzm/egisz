...
cJSON *egisz_refs_list(); //Получение списка справочников

cJSON *egisz_refs_parts //Возвращает количество частей справочника, которое можно получить
( 
		int id //Идентификатор справочника
); 

cJSON *egisz_refs_part //Возвращает записи справочника по его идентификатору
(
		int id,  //Идентификатор справочника 
...
cJSON *egisz_org_part //Возвращает часть записей организаций по типу
(
		int orgTypeId, //Тип организации 
		int part //Часть записей организаций, которую необходимо получить
); 

cJSON *egisz_org_parts //Возвращает количество частей записей организаций по типу
( 
		int orgTypeId //Тип организации 
); 

cJSON *egisz_org_get //Возвращает базовые сведения об организации по ее идентификатору (oid)
(
...
cJSON *egisz_depart_parts //Возвращает количество частей записей подразделений по типу
(
		int departTypeId //Тип подразделения
); 

cJSON *egisz_depart_part //Возвращает часть записей подразделений по типу
(
		int departTypeId, //Тип подразделения 
		int part //Часть записей подразделений, которую необходимо получить
); 

cJSON *egisz_depart_get //Возвращает сведения о подразделении по идентификатору (oid)
(
...
cJSON *egisz_mo //Возвращает сведения о подразделении по идентификатору (oid)
(
...
(
		int service //Идентификатор организации (oid)
); 
